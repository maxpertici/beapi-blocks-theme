...
class Custom_Menu_Walker extends \Walker_Nav_Menu {
	...
	public function start_el( &$output, $item, $depth = 0, $args = [], $id = 0 ) {
		...
		if ( in_array( 'menu-item-has-children', $item->classes, true ) ) {
			...
			$output .= sprintf( '<span class="aria-expanded-false-text">%s</span>', esc_html__( 'Open menu', 'beapi-blocks-theme' ) );
			$output .= sprintf( '<span class="aria-expanded-true-text">%s</span>', esc_html__( 'Close menu', 'beapi-blocks-theme' ) );
			$output .= '</button>';
