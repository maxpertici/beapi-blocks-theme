...
class WebpackThemeJsonPlugin {
	...
	constructor(options) {
		...
		this._output =
			options.output || path.resolve(__dirname, '../theme.json');
		this._scssOutput =
			options.scssOutput ||
			path.resolve(
				__dirname,
		...
		jsonFiles.forEach((file) => {
			if (file.isFile() && file.name.endsWith('.json')) {
				...
				if (isPlainObject(json)) {
					...
					console.error(
						logId,
						'JSON file is not a plain object:',
						file.name
					);
				}
			}
	...
	generateScssVariables() {
		...
		function traverse(obj, parents = [], result = '') {
			for (const key in obj) {
				const id =
					(parents.length > 0 ? parents.join('-') + '-' : '') + key;
				const taskName = taskNames.filter((t) =>
					id.startsWith(t) ? t : null
				)[0];
