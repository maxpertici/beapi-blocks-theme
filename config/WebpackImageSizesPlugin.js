...
class WebpackImageSizesPlugin {
	constructor(options) {
		// folders
		this._confImgFolder =
			path.resolve(__dirname, '../assets/conf-img') + '/';
		this._tplFolder = this._confImgFolder + 'tpl/';
		this._defaultImagesFolder =
		...
		this._imageDefault =
			path.resolve(__dirname, '../src/img/static') + '/default.jpg';
	...
	generateImageJsonFiles() {
		 ...
		 * @return {string} Default image name
		 */
		function getDefaultImgName(size) {
			return `${size.replace('img', 'default')}.jpg`;
		}

		/**
		 * Check if srcset is retina
		 * @param {string} src
		 * @return {Array} Retina array
		 */
		...
		function imageLocationsFromTpl() {
			...
			templateFileNames.forEach((tplName) => {
				...
				srcsetArr.forEach((src) => {
					...
					const crop = !(
						cropArr && cropArr[0] === 'data-crop="false"'
					);
					...
					dimensions.forEach((size, index) => {
						...
						if (sizes[size] && sizes[size].crop !== crop) {
							...
							console.log(
								logId,
								'\nSize already exists but crop is not equal :',
								size
							);
						}
						...
						that._defaultImages[getDefaultImgName(size)] =
							sizes[size];
					});
				...
				sharp(this._imageDefault)
					...
					.toFile(outputFile, (err, info) => {
						if (err) {
							...
							console.log(
								logId,
								`Created ${info.width}x${info.height} image`
							);
							// eslint-disable-next-line no-console
							console.log(
								logId,
								'at',
								outputFile.split('/themes/')[1] || ''
							);
						}
					});
			} catch (err) {
