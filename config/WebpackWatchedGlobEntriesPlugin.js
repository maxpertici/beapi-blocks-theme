/**
 * Le plugin a était modifié car il ne supportait pas les fichiers avec le même nom mais avec une extension différente.
 * Voir l'issue : https://github.com/Milanzor/webpack-watched-glob-entries-plugin/issues/48
 * Repo du plugin hacké : https://github.com/n-langle/webpack-watched-glob-entries-plugin
 */
// Fetch depencies
	...
	static getEntries(globs, globOptions, pluginOptions_) {
		...
		return function () {
			...
			globs.forEach(function (globString) {
				...
				const files = WebpackWatchedGlobEntries.getFiles(
					globString,
					globOptions,
	...
	afterCompile(compilation, callback) {
		if (Array.isArray(compilation.contextDependencies)) {
			...
			compilation.contextDependencies =
				compilation.contextDependencies.concat(directories);
		} else {
			// Support Webpack >= 4
